...
def preprocessing_data(word_query, number_of_tweets, function_option):
  ...
  data  = pd.DataFrame([tweet.full_text for tweet in posts], columns=['Tweets']) #Fetching Raw Data from Twitter
  ...
  discard = ["CNFTGiveaway", "GIVEAWAYPrizes", "Giveaway", "Airdrop", "GIVEAWAY", "makemoneyonline", "affiliatemarketing", "GiveawayWinner"]
  data = data[~data["Tweets"].str.contains('|'.join(discard))] #remove all the keywords which having discard keywords
  ...
  return data #Returing clean and formatted Data


def download_data(data, label):
    current_time = datetime.datetime.now(pytz.timezone('Asia/Kolkata'))
    current_time = "_{}_{}-{}-{}".format(current_time.date(), current_time.hour, current_time.minute, current_time.second)
